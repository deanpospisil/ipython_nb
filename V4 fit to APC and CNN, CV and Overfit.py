...
cor_v4_models_dirfit = np.array(cor_v4_models, v4_resp_apc[i])
...
# In[8]:
#lets make a nice dataframe
sel_models = []
cor_models = []
for model, model_sel, cor_modelc in zip(models, model_ind_lists, cor_v4_models_dirfit):
    sel_models.append(model[:, model_sel])

model_ind = 1    
cnn_does_better = np.argsort(cor_v4_models_dirfit[model_ind]-cor_v4_models_dirfit[2])[::-1]

for i in cnn_does_better[:10]:
    plt.figure()
    ax = plt.subplot(111)
...
# The difference between the trained net and the apc model are largely maintained when over fitting vs cross validation. Over fitting does tend to have a tighter distribution.
